    ...
    if (pleins.length > 1) {
      pleins.forEach((plein, index) => {
        if (index > 0 && index < pleins.length - 1) {
          const nbJours =
            (new Date(plein.date) - new Date(pleins[0].date)) /
            (1000 * 60 * 60 * 24);
          const cout = pleins
            .slice(0, index)
            .reduce((acc, plein) => acc + Number(plein.prix), 0);
          const coutParJour =
            nbJours > 0 ? Number((cout / nbJours).toFixed(2)) : 0;
            ...
            <Card.Title className="flex-grow-1 p-2">
              <div className="">
                <p>
                  ...
                  >
                    {coutJournalier.length >= 1
                      ? coutJournalier[coutJournalier.length - 1].cout.toFixed(
                          2,
                        )
                      : '...'}
                  </span>
